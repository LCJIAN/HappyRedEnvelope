...
public abstract class RecyclerFragment<T> extends BaseFragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        mAdapter = onCreateAdapter(mData);
    ...
    private void getData() {
        ...
        mSubscription = mObservable.subscribe(new Action1<PageResult<T>>() {
            @Override
            public void call(PageResult<T> moviePageResult) {
                mLoadMoreAdapter.setState(mPrePage < moviePageResult.total_pages ? LoadMoreAdapter.STATE_DEFAULT : LoadMoreAdapter.STATE_END);
                if (mCurrentPage == 1) {
                    setRefreshing(false);
                }

                if (mCurrentPage <= moviePageResult.total_pages && mCurrentPage == mPrePage) {
                    if (mCurrentPage == 1) {
                        mData.clear();
                        mData.addAll(moviePageResult.elements);
